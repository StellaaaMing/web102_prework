...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++){
        ...
        // about each game
        gameCard.innerHTML += `<p>Name: ${game.name}</p>`;
        gameCard.innerHTML += `<p>Description: ${game.description}</p>`; 
        gameCard.innerHTML += `<img class="game-img" src="${game.img}">`;
...
let totalContribute = GAMES_JSON.reduce((acc, game)=>acc + game.backers, 0);
...
let strExpUnfund=`Currently there are ${unFundGames.length} games with insufficient funding and need help!`;
